...
function Balloon:update(dt, objects)
    ...
    -- Check for collisions with sharp objects (Fans, etc.)
    for _, obj in ipairs(objects) do
        if obj.type == "fan" and obj.active then
            local bx, by = self.body:getPosition()
            local fx, fy = obj.x, obj.y
            local dist = ((bx - fx)^2 + (by - fy)^2) ^ 0.5
            if dist < 60 and not self.popped then
                self.popped = true
                physics.world:destroyBody(self.body)
                break
            end
        end
    end
